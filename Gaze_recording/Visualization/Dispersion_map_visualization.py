...
def Dispersion_map(gaze_x, gaze_y, grid, detail):
...
for i in range(nb_image): # LOOP OVER IMAGES
    ...
    """
    ################## Dispersion map plot #######################
    if len(gaze_x_htmp) != 0 and PLOT_CHOICE == 'Dispersion_map': # if data exist
        # --- Display the histogram overlap on the reference image --- #
       dispersion_map(img_list[i],gaze_x_raw,gaze_y_raw,FILENAME,extent,img,win_size)
    """
    ################## Raw gaze data plot ##################
    ...
    if len(gaze_x_raw) != 0 and PLOT_CHOICE == 'Both':
        ...
        Disper_raw_plot(img_list[i],gaze_x_raw,gaze_y_raw,FILENAME,extent,img,win_size,cmap)
    """