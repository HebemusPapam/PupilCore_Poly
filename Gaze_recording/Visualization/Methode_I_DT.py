...
def Check_Fixation_Salvucci(time,start,end,duration_limit,rayon_dispersion,All_Points_Not_Searched,x,y,radius):
...
def Check_Micro_Saccade_Salvucci(MicroS,t0,t1,x,y,microS_info,time):
    ...
    while(t1-t0<MicroS_duration and All_Points_Not_Searched and not MicroS):
        ...
        if abs(calcul_disp(x1,y1)-calcul_disp(x2,y2)) < 30:
            MicroS = True
            ...
            print(" slice :  ",slice(t0,t1,1),"\n\r")
            print(" x:  ",x, "\n\r")
            np.delete(x,slice(t0,t1,1),axis=0)
            np.delete(y,slice(t0,t1,1),axis=0)
...
def dispersion_map_Salvucci(time,gaze_x,gaze_y,radius,duration):
    ...
    while (All_Points_Not_Searched):
        ...
        rayon_dispersion,All_Points_Not_Searched,dispersion,start,end = Check_Fixation_Salvucci(time,start,end,duration_limit,rayon_dispersion,All_Points_Not_Searched,gaze_x,gaze_y,radius)
        ...
        if rayon_dispersion!=radius:
            ...
            Micro_Saccade,gaze_x,gaze_y,All_Points_Not_Searched,saccade = Check_Micro_Saccade_Salvucci(Micro_Saccade,start,end,gaze_x,gaze_y,saccade,time)
            
            if Micro_Saccade :
                print("Micro")
            #Il y a une fixation mais pas de micro saccade
            else:
                print("Dipserion : ",dispersion," start: ",start, "  end : ",end)
            ...
            #On a besoin d'une saccade entre deux dispersion
            while(No_Saccade and All_Points_Not_Searched):
                #########Condition Temporel sur une saccade############
                #une saccade ne doit pas être confondu avec un blink donc t > 20ms
                while((time[end]-time[start])<0.02 and All_Points_Not_Searched):
                    if end<(len(gaze_x)-2):
                        end+=1
                    else:
                        All_Points_Not_Searched = False
    ...
    print("Cerlce Salvador : ",fixation)
