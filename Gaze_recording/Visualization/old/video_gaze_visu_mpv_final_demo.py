...
def get_mpv_options(slowdown=1.0):
...
is_running=True
def get_new_mpv_player(options):
    "create a player, with callbacks attached"
    ...
    def time_observer(_name, value):
        ...
        if value:
            plot_frame_gazes(int(value*1000.5))
        #~ print('Now playing at {:.2f}s'.format(value))

    player.loop_playlist = False # 'inf'
...
for i in range(1):
    ...
    # --- mpv player settings --- #
    # create a player, with its options
    mpv_opt=get_mpv_options(slowdown=1.0)
    mpv_opt['force_window']=True
    player=get_new_mpv_player(mpv_opt)

    # load file, possibly in pause
    #player.pause=not opt_run
